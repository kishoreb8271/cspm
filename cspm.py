...
if not st.session_state['authenticated']:
                ...
                if provider == "AWS":
                    try:
                        ...
                        for b in buckets:
                            ...
                            # 2. REAL-TIME DSPM: Object Content Inspection
                            objects = s3.list_objects_v2(Bucket=b_name, MaxKeys=5)
                            if 'Contents' in objects:
                                for obj in objects['Contents']:
                                    ...
                                    # Regex for AWS Secret Key pattern
                                    if re.search(r'([^A-Z0-9])[A-Z0-9]{40}(?![A-Z0-9])', content):
                                        dspm_data.append({
                                            "Resource": f"s3://{b_name}/{file_key}", 
                                            "File_Name": file_key.split('/')[-1],
                                            "Location": f"{b_name}/data/", 
                                            "Type": "S3 Object", 
                                            "Severity": "Critical", 
                                            "Issue": "Real-time Secret Exposure", 
    ...
    # --- MAIN TABS ---
    tabs_list = ["🤖 AI CNAPP Dashboard", "📊 Executive Dashboard", "🔌 Cloud Integration", 
    ...
    with active_tab[1]:
        ...
        st.caption(f"⏱️ Last Scan: {st.session_state['last_scan_time']}")
        ...
        with m2: st.metric("High Risk", len(all_findings[all_findings['Severity'] == 'High']) if not all_findings.empty else 0)
    ...
    with active_tab[2]:
        ...
        with col_left:
            provider_choice = st.selectbox("Provider", ["AWS", "Azure"])
    ...
    with active_tab[6]:
        st.header("🛡️ Real-Time DSPM")
        st.info("Scanning live S3 object content for sensitive data leaks.")
