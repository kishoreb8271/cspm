...
if not st.session_state['authenticated']:
    ...
    def run_real_time_scan(module_name="Full System"):
        if not st.session_state['integrations']:
            # Suppress warning during silent real-time background cycles
            if "Real-Time" not in module_name:
        ...
        # Only show the status UI for manual or scheduled full scans
        with st.status(f"🚀 Running {module_name} Scan...", expanded=("Real-Time" not in module_name)) as status:
