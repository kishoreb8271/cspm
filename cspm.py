...
# This fragment runs every 60 seconds to check if a scheduled scan is due
...
def background_heartbeat():
    if st.session_state.get('schedule_enabled') and st.session_state.get('next_scan_time'):
        if datetime.datetime.now() >= st.session_state['next_scan_time']:
            # We call the scan function logic directly here
            run_real_time_scan("Scheduled")
            # The interval is retrieved from the session state or a default
...
if not st.session_state['authenticated']:
    ...
    if 'current_interval_hours' not in st.session_state:
        st.session_state['current_interval_hours'] = 1
        ...
        with col_right:
            ...
            if not st.session_state['schedule_enabled']:
                if st.button("Enable Scheduler", type="primary"):
                    ...
                    st.session_state['current_interval_hours'] = interval_hours
                    st.session_state['next_scan_time'] = datetime.datetime.now() + datetime.timedelta(hours=interval_hours)
                    st.rerun()
            else:
